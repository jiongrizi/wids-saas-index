      ...
      <el-col :span="24">
        <el-row type="flex" class="row-bg" justify="space-between">
          ...
          <el-col :span="12" class="text-align-right">
            <a class="el-button el-button--primary is-plain" :href="'/#/sys-web-name-list'">返回</a>
          </el-col>
        </el-row>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    getData() {
      var t = this;
      var postUrl = "/sys/waf_get_sys_web_name_list";
    ...
    onClickWebRuleProSubmit(sysNameListForm) {
      ...
      var postUrl = "/sys/waf_edit_sys_web_name_list";
      if (t.type == 'new') {
        postUrl = "/sys/waf_create_sys_web_name_list";
      } else {
      ...
      this.$refs[sysNameListForm].validate((valid) => {
        if (valid) {
          ...
          JXAjax(
            ...
            function (response) {
              t.loading = false;
              window.location.href = '/#/sys-web-name-list/'
            },
            function () {
              t.loading = false;
            }
