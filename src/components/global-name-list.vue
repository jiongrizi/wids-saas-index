    ...
    <el-row v-loading.fullscreen.lock="loadingPage">
      <el-col :span="24">
        <el-row class="text-align-right">
          ...
          <el-button type="primary" @click="onClickDownloadRule()" :loading="loadingDownloadRule">加载名单</el-button>
...
export default {
  ...
  methods: {
    getData() {
      var t = this;
      var postUrl = "/wids/waf_get_base_name_list_list";
      var oData = {  };
      JXAjax(
        "post",
        postUrl,
    ...
    onClickDownloadRule() { 
      ...
      JXAjax(
        "post",
        "/sys/waf_get_sys_name_list_list",
        {  },
        function (response) {
    ...
    getDifference(arr1, arr2) {
      ...
      arr2.forEach(item => {
        var flag = true;
        arr1.forEach(i => {
          if (i.name_list_uuid == item.name_list_uuid) {
            flag = false
          }
        })
        if (flag) {
    ...
    onClickDownloadRuleSubmit(downloadRuleForm) { 
      var t = this;
      var postUrl = "/wids/waf_load_base_name_list";
      var oData = { name_list_uuid: t.downloadRuleForm.name_list_uuid };
    ...
    handleDelete(data) {
      var t = this;
      t.loading = true;
      var postUrl = "/wids/waf_del_base_name_list";
      var oData = {
    ...
    onClickChangeOrderSubmit(index, value, operate) { 
      ...
      var oData = {
        name_list_uuid: value.name_list_uuid,
      };
      if (index > 0) { 
        if (operate == "top") {
          oData.type = "top";
        ...
        if (operate == "up") {
          oData.type = "exchange";
          oData.exchange_name_list_uuid = t.tableData[index - 1].name_list_uuid;
        }
      }
      ...
      var postUrl = "/wids/waf_exchange_base_name_list_priority";
    ...
    onChangeRuleStatus(value) { 
      ...
      var postUrl = "/wids/waf_edit_base_name_list";
      JXAjax(
        "post",
        postUrl,
        oData,
