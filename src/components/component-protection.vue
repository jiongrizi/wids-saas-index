    ...
    <el-row v-loading.fullscreen.lock="loadingPage">
      <el-col :span="24">
        <el-row class="text-align-right">
          ...
          <el-button type="primary" @click="onClickDownloadRule()" :loading="loadingDownloadRule">加载组件</el-button>
      ...
      <el-dialog v-model="dialogDownloadRuleFormVisible" title="加载" :close-on-click-modal="false" width="520px"
        ...
        <el-form class="form-download-rule-dialog" :model="downloadRuleForm" label-position="left" label-width="130px"
          ...
          <el-form-item label="加载组件" key="1" prop="uuid">
            <el-select v-model="downloadRuleForm.uuid" placeholder="请选择或输入模糊搜索" filterable>
              <el-option v-for="item in ruleOptions" :key="item.uuid" :label="item.name"
                :value="item.uuid">
              </el-option>
            </el-select>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      downloadRuleForm: [],
      ruleOptions: [],
      dialogLookRuleFormVisible: false,
      lookRuleForm: [],
    };
  },
  computed: {
  ...
  methods: {
    getData() {
      var t = this;
      var postUrl = "/waf/waf_get_component_protection_list";
      var oData = { domain: t.domain };
      if (t.ruleType == "group_rule") {
        postUrl = "/waf/waf_get_group_component_protection_list";
    ...
    onClickDownloadRule() { 
      ...
      JXAjax(
        "post",
        "/waf/waf_get_sys_component_protection_list",
        {  },
        function (response) {
    ...
    onClickDownloadRuleSubmit(downloadRuleForm) { 
      var t = this;
      var postUrl = "/waf/waf_load_component_protection";
      var oData = { uuid: t.downloadRuleForm.uuid, domain: t.domain };
      if (t.ruleType == "group_rule") {
        postUrl = "/waf/waf_load_group_component_protection";
        oData = { uuid: t.downloadRuleForm.uuid, group_id: t.domain};
      }
    ...
    handleDelete(data) {
      var t = this;
      t.loading = true;
      var postUrl = "/waf/waf_del_component_protection";
      var oData = {
        domain: t.domain,
        uuid: data.uuid
      };
      if (t.ruleType == "group_rule") {
        postUrl = "/waf/waf_del_group_component_protection";
        oData = {
          group_id: t.domain,
          uuid: data.uuid
        };
    ...
    onClickChangeOrderSubmit(index, value, operate) { 
      ...
      if (index > 0) { 
        ...
        if (operate == "up") {
          oData.type = "exchange";
          oData.exchange_uuid = t.tableData[index - 1].uuid;
        }
      }
      ...
      var postUrl = "/waf/waf_exchange_component_protection_priority";

      if (t.ruleType == "group_rule") {
        postUrl = "/waf/waf_exchange_group_component_protection_priority";
        oData.group_id = t.domain;
      } else {
    ...
    onChangeRuleStatus(value) { 
      ...
      var postUrl = "/waf/waf_edit_component_protection_status";
      if (t.ruleType == "group_rule") {
        postUrl = "/waf/waf_edit_group_component_protection_status";
        oData = {
          group_id: t.domain,
          uuid: value.uuid,
          status: value.status
