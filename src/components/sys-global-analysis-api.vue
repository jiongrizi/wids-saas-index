      ...
      <el-col :span="24">
        <el-row class="text-align-right">
          <div class="domain-search-input">
            <el-input placeholder="请输入组件名称进行搜索" prefix-icon="el-icon-search" v-model="domainSearch"></el-input>
          </div>
          <a class="el-button el-button--primary" :href="'/#/sys-global-analysis-api-edit/new'">新建API</a>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    handleEdit(data) {
      window.location.href = "/#/sys-global-analysis-api-edit/" + data.api_name;
    },
    handleDelete(data) {
      var t = this;
      if (data.status == 'true') { 
      ...
      JXAjax(
        "post",
        "/sys/waf_delete_sys_global_analysis_api",
        {
